...
def OCRRemoteOnConnect(is_connected: c_bool, user_data: py_object):
    logger.debug(
        f"OCRRemoteOnConnect 回调函数被调用, 参数, is_connected: {is_connected}"
    )
    if user_data:
...
def OCRRemoteOnDisConnect(user_data: py_object):
    logger.debug(f"OCRRemoteOnDisConnect 回调函数被调用 ")
    if user_data:
        manager_obj: OcrManager = cast(user_data, py_object).value
...
def OCRReadOnPush(request_id: c_uint32, request_info: c_void_p, user_data: py_object):
    logger.debug(
        f"OCRReadOnPush 回调函数被调用 参数, request_id: {request_id}, request_info: {request_info}"
    )
    if user_data:
        manager_obj: OcrManager = cast(user_data, py_object).value
        pb_size = c_uint32()
        ...
        if pb_size.value > 10:
            logger.debug(f"正在解析pb数据，pb数据大小: {pb_size.value}")
...
class OcrManager(XPluginManager):
    ...
    def __init__(self, wechat_path) -> None:
        ...
        self.AppendSwitchNativeCmdLine("user-lib-dir", wechat_path)
        for i in range(1, 33):
    ...
    def DoOCRTask(self, pic_path: str):
        ...
        while not self.m_connect_state.value:
            logger.debug("等待Ocr服务连接成功!")
            time.sleep(1)
        _id = self.GetIdleTaskId()
        if not _id:
            raise Exception("当前队列已满，请等待后重试")
        return self.SendOCRTask(_id, pic_path)
    ...
    def SendOCRTask(self, task_id: int, pic_path: str):
        ...
        self.SendPbSerializedData(
            serialized_data,
            len(serialized_data),
            MMMojoInfoMethod.kMMPush.value,
            0,
            RequestIdOCR.OCRPush.value,
        )
        ms = event.get()
        del self.m_id_path[task_id]
        self.SetTaskIdIdle(task_id)
        return ms

    def CallUsrCallback(
        self, request_id: c_uint32, serialized_data: c_void_p, data_size: int
    ):
        ...
        if not self.m_id_path.get(task_id):
            return
        event: TaskEvent = self.m_id_path[task_id]
        event.set(self.parse_json_response(json_response_str))
    ...
    def parse_json_response(self, json_response_str: str):
        ...
        sorted(results["ocrResult"], key=self.sort)
        return results
